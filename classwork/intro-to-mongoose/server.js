...
db.on('error', err=> console.group(err.message + 'is mongod not running?'));
...
// Tweet.create(myFirstTweet).then(createdTweet => console.log(createdTweet)).catch(error => console.error(error)).finally(() => db.close())
...
Tweet.find({}, "title body").then(allTweets => console.log(allTweets)).catch(error => console.error(error)).finally(() => db.close())